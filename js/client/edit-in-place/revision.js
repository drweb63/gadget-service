var uRevision = function (info) {
	...
	*/
	this.save = function (callback) {
		...
		if(deleteAction) {
			jQuery.post('/admin/content/eip_del_page.xml' + qSuffix, {'element-id': elementId, 'object-id': objectId}, function (data) {
		...
		if(typeof newValue == 'object') {
			...
			if (typeof newValue == 'string') {
				if (jQuery.browser.mozilla && newValue.match(/="\.\.\//g)) {
					newValue = newValue.replace(/="[\.\.\/]+/g, '="/');
				}
				newValue = newValue.replace(/\sumi:[-a-z]+="[^"]*"/g, '');
			}
