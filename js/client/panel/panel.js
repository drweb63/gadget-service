var uPanel = function (_params) {
	...
	var renderPanel = function () {
		...
		jQuery('#u-quickpanel #edit, #on_edit_in_place').click(function() {
			self.swapEditor();
			return false;
		});
		...
		if(jQuery.cookie('eip-editor-state')) {
			...
			// Редактирование страницы не включено
			window.ticketsControl.enable(); // разрешаем показывать тикеты
			window.editInAdmin.enable(); // разрешаем переход на редактирование в админке
		}
	...
	var uPageEditorOnInit = function () {
		// Ловим нажатие клавиши F2
		jQuery(document).bind('keydown', function(e) {
			if(e.keyCode == 113) {
				uPanel.prototype.swapEditor();
			}
		});
...
uPanel.prototype.swapEditor = function () {
	...
	if(editor.isEnabled()) {
		...
		window.ticketsControl.enable();
		window.editInAdmin.enable();
	} else {
		editor.enable();
		...
		window.ticketsControl.disable();
		window.editInAdmin.disable();
	}
};
